...
class Program
{
    public static string connectionString = @"Data Source=HabitTracker.Rutwik2005.db";
    ...
    {
        string date = GetDateInput();  // Get the date input
    ...
    private static void Delete()
    {
        Console.Clear();
        GetAllRecords();
        var recordId = GetNumberInput("\n\nPlease type the ID of the record you want to delete or type 0 to go back to Mian menu\n\n");
        using (var connection = new SqliteConnection(connectionString))
        {
            connection.Open();
            var tableCmd = connection.CreateCommand();
