...
void GetInput()
{
	...
	while (closeApp == false)
	{
        ...
        Console.WriteLine("Type 5 to See Records from the last seven days");
		...
		switch (input)
		{
			...
			case "4": UpdateRecord(); Console.Clear();
				break;
			case "5": LastSevenDays(); Console.ReadLine(); Console.Clear();
				break;
			default:
...
void LastSevenDays()
{
	...
	DateTime beginDate = todaysDate.AddDays(-7);

	using (var connection = new SqliteConnection(connectionString))
	{
		connection.Open();
		var command = connection.CreateCommand();
		command.CommandText = $"SELECT * FROM habit WHERE Date >= '{beginDate.ToString("dd/MM/yy")}' ORDER BY Date";
...
string GetDate(string question)
{
	...
	while (!DateTime.TryParseExact(input, "dd/MM/yy", new CultureInfo("nl-BE"), DateTimeStyles.None, out _) || Convert.ToDateTime(input) > DateTime.Now)
	{
		
		Console.Write("Invalid date or date in the future, please enter a valid date: ");
		input = Console.ReadLine();
	}
